...
data %<>% filter(data, !is.na(EMS_Code))

data %<>% separate(EMS_Code, paste0("EMS_Code", 1:5))

data %<>% pivot_longer(paste0("EMS_Code", 1:5), names_to = "EMS_Code_Number", values_to = "EMS_Code")
